            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <Input
                  ...
                  className={`bg-slate-700 border-slate-600 text-white ${showCpfError ? "border-red-500 focus:border-red-500" : ""
                    }`}
                  maxLength={14}
                  required
                />
          ...
          <Card className="bg-slate-800/50 border-slate-700 animate-fade-in">
            ...
            <CardContent className="space-y-8">
              ...
              >
                ...
                <RadioGroup
                  value={formData[questions[currentQuestion].id as keyof typeof formData] as string}
                  onValueChange={(value) =>
                    handleInputChange(questions[currentQuestion].id, value)
                  }
                  className="space-y-3"
                >
                  {questions[currentQuestion].options.map(
                    (option, optionIndex) => (
                      ...
                      >
                        <RadioGroupItem
                          value={option}
                          id={`${questions[currentQuestion].id}-${optionIndex}`}
                          className="border-white text-white mt-1 flex-shrink-0"
                        />
                        <Label
                          htmlFor={`${questions[currentQuestion].id}-${optionIndex}`}
              ...
              <div className="flex items-center justify-between pt-6">
                ...
                {currentQuestion < questions.length - 1 ? (
                  <Button
                    ...
                    onClick={handleNext}
                    disabled={
                      !formData[questions[currentQuestion].id as keyof typeof formData] || isSubmitting
                    }
                  >
                    Próxima <ChevronRight className="ml-2 h-5 w-5" />
                  </Button>
                  ...
                  <Button
                    type="submit"
                    className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold px-8 py-3 text-lg min-w-[150px]"
                    disabled={
                      !formData[questions[currentQuestion].id as keyof typeof formData] || isSubmitting
                    }
                  >
